...
3. 只有用户亲自明确提出的艺人、语言、年代、数量和排除词才属于硬条件。不要自行添加歌手、类型、语言或年代限制。
4. 如果用户明确要求歌曲数量，严格遵守；符合条件的歌曲不足时只返回实际符合的数量。用户没有指定数量时，不设置固定首数，以完成一段约 45–75 分钟、听感完整的旅程为目标，根据资料库内容自由决定长度，通常可选择 12–20 首。
